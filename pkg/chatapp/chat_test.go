...
func TestRuntimeEventSinkPublishesAgentModePreview(t *testing.T) {
	...
	sink := &runtimeEventSink{sessionID: sessionstream.SessionId("chat-preview"), messageID: "chat-msg-1", pub: pub, engine: NewEngine()}
...
func TestRuntimeEventSinkPublishesAgentModeCommitted(t *testing.T) {
	pub := &capturingPublisher{}
	sink := &runtimeEventSink{sessionID: sessionstream.SessionId("chat-commit"), messageID: "chat-msg-2", pub: pub, engine: NewEngine()}
...
func TestUIProjectionEmitsAgentModePreviewAndCommitEvents(t *testing.T) {
	...
	require.NoError(t, err)
	previewEvents, err := uiProjection(context.Background(), sessionstream.Event{Name: EventAgentModePreview, SessionId: "sid", Ordinal: 7, Payload: previewPayload}, nil, nil)
	require.NoError(t, err)
	...
	require.NoError(t, err)
	commitEvents, err := uiProjection(context.Background(), sessionstream.Event{Name: EventAgentModeCommitted, SessionId: "sid", Ordinal: 8, Payload: commitPayload}, nil, nil)
	require.NoError(t, err)
...
func TestTimelineProjectionPersistsCommittedAgentModeEntity(t *testing.T) {
	...
	require.NoError(t, err)
	entities, err := timelineProjection(context.Background(), sessionstream.Event{Name: EventAgentModeCommitted, SessionId: "sid", Ordinal: 9, Payload: payload}, nil, staticTimelineView{})
	require.NoError(t, err)
...
func (p *capturingPublisher) Publish(_ context.Context, ev sessionstream.Event) error {
