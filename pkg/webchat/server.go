...
func NewServer(ctx context.Context, parsed *layers.ParsedLayers, staticFS embed.FS) (*Server, error) {
    ...
    if err != nil { return nil, errors.Wrap(err, "build http server") }
...
func (s *Server) Run(ctx context.Context) error {
    ...
    eg.Go(func() error {
        ...
        defer cancel()
        if err := s.httpSrv.Shutdown(shutdownCtx); err != nil { log.Error().Err(err).Msg("server shutdown error"); return err }
        if err := s.router.router.Close(); err != nil { log.Error().Err(err).Msg("router close error") } else { log.Info().Msg("router closed") }
