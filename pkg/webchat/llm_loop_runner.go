...
func (r *LLMLoopRunner) Start(ctx context.Context, req StartRequest) (StartResult, error) {
	...
	if conv == nil || conv.Sess == nil {
		return StartResult{}, errors.New("llm loop runner conversation is invalid")
	}
	...
	seed, err := conv.Sess.AppendNewTurnFromUserPrompt(payload.Prompt)
	if err != nil {
	...
	handle, err := conv.Sess.StartInference(baseCtx)
	if err != nil {
		return StartResult{}, err
	}
	if handle == nil {
