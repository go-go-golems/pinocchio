...
type ConvManager struct { mu sync.Mutex; conns map[string]*Conversation }
...
func (r *Router) startReader(conv *Conversation) error {
    ...
    if err != nil { readCancel(); conv.stopRead = nil; return err }
    conv.reading = true
    go func() {
        for msg := range ch {
            e, err := events.NewEventFromJson(msg.Payload)
            if err != nil { log.Warn().Err(err).Str("component", "ws_reader").Msg("failed to decode event json"); msg.Ack(); continue }
            runID := e.Metadata().RunID
            if runID != "" && runID != conv.RunID { msg.Ack(); continue }
            r.convertAndBroadcast(conv, e)
            msg.Ack()
        }
        conv.mu.Lock(); conv.reading = false; conv.stopRead = nil; conv.mu.Unlock()
...
func (r *Router) getOrCreateConv(convID string, buildEng func() (engine.Engine, *middleware.WatermillSink, message.Subscriber, error)) (*Conversation, error) {
    r.cm.mu.Lock(); defer r.cm.mu.Unlock()
    if c, ok := r.cm.conns[convID]; ok { return c, nil }
    conv := &Conversation{ID: convID, RunID: uuid.NewString(), conns: map[*websocket.Conn]bool{}}
    eng, sink, sub, err := buildEng()
    if err != nil { return nil, err }
    conv.Eng = eng; conv.Sink = sink; conv.sub = sub
    conv.Turn = &turns.Turn{RunID: conv.RunID, Data: map[string]any{}}
    if err := r.startReader(conv); err != nil { return nil, err }
    r.cm.conns[convID] = conv
    return conv, nil
}

func (r *Router) addConn(conv *Conversation, c *websocket.Conn) {
    conv.connsMu.Lock(); conv.conns[c] = true; conv.connsMu.Unlock()
    conv.mu.Lock(); if conv.idleTimer != nil { conv.idleTimer.Stop(); conv.idleTimer = nil }; wasReading := conv.reading; conv.mu.Unlock()
    if !wasReading && r.usesRedis { _ = r.startReader(conv) }
}

func (r *Router) removeConn(conv *Conversation, c *websocket.Conn) {
    conv.connsMu.Lock(); delete(conv.conns, c); conv.connsMu.Unlock(); _ = c.Close()
    if r.idleTimeoutSec <= 0 { return }
    conv.connsMu.RLock(); empty := len(conv.conns) == 0; conv.connsMu.RUnlock()
    if !empty { return }
    conv.mu.Lock()
    if conv.idleTimer == nil {
        d := time.Duration(r.idleTimeoutSec) * time.Second
        conv.idleTimer = time.AfterFunc(d, func() {
            conv.mu.Lock(); defer conv.mu.Unlock()
            conv.connsMu.RLock(); isEmpty := len(conv.conns) == 0; conv.connsMu.RUnlock()
            if isEmpty && conv.stopRead != nil { conv.stopRead(); conv.stopRead = nil; conv.reading = false }
        })
    }
    conv.mu.Unlock()
}
