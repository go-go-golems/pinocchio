...
func (cs *ChatSession) runChatInternal() error {
    ...
    eg.Go(func() error {
        ...
        // Seed backend from the provided seed turn (system/history) so chat UI reflects prior run
        if cs.seedTurn != nil {
            backend.SetSeedTurn(cs.seedTurn)
            log.Debug().Str("component", "chatrunner").Msg("Seeded backend from seed turn for chat UI")
        }
...
func (cs *ChatSession) runBlockingInternal() error {
    ...
    // Use provided seedTurn or start fresh
    seed := cs.seedTurn
    if seed == nil { seed = &turns.Turn{} }
    ...
    // Print the last assistant message from updated turn to the output writer
    if updatedTurn != nil && len(updatedTurn.Blocks) > 0 {
        for i := len(updatedTurn.Blocks) - 1; i >= 0; i-- {
            b := updatedTurn.Blocks[i]
            if b.Kind == turns.BlockKindLLMText {
                if txt, ok := b.Payload[turns.PayloadKeyText].(string); ok {
                    if _, err := fmt.Fprintln(cs.outputWriter, txt); err != nil {
...
// WithSeedTurn sets the initial Turn to seed the UI and blocking run.
func (b *ChatBuilder) WithSeedTurn(t *turns.Turn) *ChatBuilder {
    if b.err != nil {
        return b
    }
    if t == nil {
        b.err = errors.New("seed turn cannot be nil")
        return b
    }
    b.seedTurn = t
    return b
