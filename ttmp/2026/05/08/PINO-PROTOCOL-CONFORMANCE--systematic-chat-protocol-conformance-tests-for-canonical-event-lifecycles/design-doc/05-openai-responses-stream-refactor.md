...
1. Add this design doc and update ticket tasks/diary/changelog.
2. Extract Responses terminal types and `responsesStreamState`.
3. Move provider-call, segment, and tool correlation closures onto state methods.
4. Extract final metadata update into `finalizeResponsesStreamMetadata`.
5. Extract final assistant/tool turn-block appending into `appendResponsesFinalTurnBlocks`.
6. Extract provider-call finish emission into `finishResponsesProviderCall`.
7. Extract SSE reading/flush into `consumeResponsesStream` while preserving behavior.
8. Wire `runStreamingInference` to the same visible shape as Chat Completions.
9. Add small table-driven helper tests only for new helper behavior that is not already protected.
10. Run `go test ./pkg/steps/ai/openai_responses -count=1` and then Geppetto pre-commit/full validation at commit points.
11. Update diary/tasks/changelog and commit each stable checkpoint.
