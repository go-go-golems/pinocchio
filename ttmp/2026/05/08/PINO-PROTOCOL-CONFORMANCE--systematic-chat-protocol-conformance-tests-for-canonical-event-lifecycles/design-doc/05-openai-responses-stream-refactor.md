...
11. Continue moving provider-event cases into named handlers only where readability improves. **Done at the first structural level: `runStreamingInference` now delegates provider-event handling to `handleResponsesProviderEvent` in `stream_events.go`; further semantic splitting is optional review follow-up.**
12. Continue moving mutable scratch state into `responsesStreamState` carefully, without large unsafe rewrites. **Done for assistant message state, response id, tool-call accumulation, terminal usage/stop/error state, and reasoning scratch state.**
13. Run `go test ./pkg/steps/ai/openai_responses -count=1` and then Geppetto pre-commit/full validation at commit points. **Done for the committed state-migration and handler-extraction checkpoints.**
