...
The architecture is deliberately split across three layers. Geppetto now owns generic OAuth protocol mechanics and request-time bearer renewal. Pinocchio owns profile format, filesystem permissions, browser callback lifecycle, and source construction. llm-proxy is a separate server host that will need a vault-backed adapter; it must not read Pinocchio’s local profile file at request time. The profile/store/runtime/login implementation is now present, but this ticket still does not claim that a real provider contract is enabled. Provider-specific authorization endpoints, exact registered loopback redirects, scopes, and refresh behavior must be documented and approved before a real login smoke. llm-proxy vault support remains separate.
