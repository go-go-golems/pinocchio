...
OUT="$OUT_DIR/ws-hookability-audit.txt"
{
  ...
  rg -n "WithTimelineUpsertHook|timelineUpsertHookOverride|RegisterTimelineHandler|WithEventSinkWrapper|WithBuildSubscriber|ConversationRequestResolver|WithConversationRequestResolver|RuntimeComposer|WithRuntimeComposer|RouterOption" "$ROOT/pinocchio/pkg/webchat" -g'*.go'
  echo
