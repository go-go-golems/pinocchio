...
RelatedFiles:
    ...
    - Path: pinocchio/pkg/webchat/engine_from_req.go
      Note: request resolver and runtime key policy for chat/ws handlers
    - Path: pinocchio/pkg/webchat/runtime_composer.go
      Note: app-owned runtime composition contract used by ConvManager
...
The websocket endpoint is implemented in `router.go` (`/ws` handler). Current request inputs are:
...
3. `WithEventSinkWrapper`, `WithBuildSubscriber`, `WithConversationRequestResolver`, `WithRuntimeComposer`: allow dependency injection around event ingestion and request/runtime policy.
...
1. `/ws` query params parsed today: `conv_id`, `runtime`.
...
Implication: the protocol event taxonomy is already rich enough to support subscription filtering without inventing new event types.
...
1. `Router` owns `*ConvManager` and request/runtime/hook dependencies.
...
Add optional debug channel frame (reference-style payload by default):
