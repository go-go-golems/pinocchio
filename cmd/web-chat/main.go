...
func (c *Command) RunIntoWriter(ctx context.Context, parsed *layers.ParsedLayers, _ io.Writer) error {
	...
	getOrCreateConv := func(convID string) (*Conversation, error) {
		...
		{
			eng_, err := factory.NewEngineFromParsedLayers(parsed)
			if err != nil {
				return nil, errors.Wrap(err, "engine init failed")
			}
			eng_ = middleware.NewEngineWithMiddleware(eng_, middleware.NewSystemPromptMiddleware("You are a helpful assistant. Be concise."))
			if s.AgentModeEnabled {
				eng_ = middleware.NewEngineWithMiddleware(eng_, agentmode.NewMiddleware(amSvc, amCfg))
			}
			eng_ = middleware.NewEngineWithMiddleware(eng_, middleware.NewToolResultReorderMiddleware())
			if dbWithRegexp != nil {
				eng_ = middleware.NewEngineWithMiddleware(eng_, sqlitetool.NewMiddleware(sqlitetool.Config{DB: dbWithRegexp, MaxRows: 500}))
			}
			conv.Eng = eng_
			conv.Sink = middleware.NewWatermillSink(router.Publisher, topicForConv(conv.ID))
		}
		// Initialize long-lived turn
		conv.Turn = &turns.Turn{RunID: conv.RunID, Data: map[string]any{}}
		if err := startReader(conv); err != nil {
			return nil, err
		}
	...
	http.HandleFunc("/chat", func(w http.ResponseWriter, r *http.Request) {
		...
		// Append user block to long-lived turn
		if conv.Turn == nil {
			conv.Turn = &turns.Turn{RunID: conv.RunID, Data: map[string]any{}}
		}
		...
		go func(conv *Conversation) {
			...
			// Ensure turn has Data map initialized to avoid nil map assignments in helpers
