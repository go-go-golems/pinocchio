...
describe('routeModeFromSearch', () => {
  ...
  it('ignores removed provider demo flags', () => {
    expect(routeModeFromSearch('?providerDemo=1')).toEqual({ kind: 'chat' });
    expect(routeModeFromSearch('?providerMultiDemo=1')).toEqual({ kind: 'chat' });
  });
  ...
  it('keeps debug priority over removed dev flags', () => {
...
describe('routeModeFromLocation', () => {
  it('reads search from a Location-like object', () => {
    expect(routeModeFromLocation({ search: '?providerDemo=1' } as Location)).toEqual({ kind: 'chat' });
  });
});
