...
The stable public parts are the values accepted by `ChatPart` in `src/webchat/types.ts`: `root`, `header`, `timeline`, `composer`, `statusbar`, `turn`, `bubble`, `content`, `composer-input`, `composer-actions`, and `send-button`.
