...
SELECT id, created_at, type, level, message, tool_name, tool_id, input, result, session_id, inference_id, turn_id
FROM chat_events
ORDER BY id DESC;
...
SELECT
  ...
  MAX(inference_id) AS inference_id,
...
-- Purpose: Summary per turn with session_id, mode from Turn.Data, injected mode, and last mode Info event
