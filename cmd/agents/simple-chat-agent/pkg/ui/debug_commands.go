	...
	case "blocks":
		...
		for i := 1; i < len(args); i++ {
			...
			case strings.HasPrefix(a, "--head="):
				_, _ = fmt.Sscanf(strings.TrimPrefix(a, "--head="), "%d", &head)
			case a == "--head" && i+1 < len(args):
				_, _ = fmt.Sscanf(args[i+1], "%d", &head)
				i++
			case strings.HasPrefix(a, "--tail="):
				_, _ = fmt.Sscanf(strings.TrimPrefix(a, "--tail="), "%d", &tail)
...
func showBlocks(turnID string, phase string, verbose bool, head, tail int) (string, error) {
	...
	for i := start; i < end; i++ {
		...
		if r.kind == 3 {
			...
			needArgs := (!r.args.Valid) || (r.args.String == "")
			if needArgs && tcid != "" {
				lookupPhase := phase
				if lookupPhase == "" {
					lookupPhase = "post_tools"
				}
